...
fn test_load_skills_seeds_examples_once() {
    ...
    // 首次加载：种子 1 个内置示例（news-digest）+ 1 个内置元 skill（skill-authoring）
    ...
    // 目录已存在时不再种子：手动删掉唯一的示例，再 load 不会恢复（元 skill 仍在）
    std::fs::remove_dir_all(skills_dir.join("news-digest")).unwrap();
    let skills = llaia::skill::loader::load_skills(&skills_dir);
...
fn test_skills_prompt_injected_with_paths() {
    ...
    let expected = skills_dir.join("news-digest").join("SKILL.md");
