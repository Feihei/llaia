...
> **实例层（ADR-0032，2026-09-23 实现）**：serve 退化为宿主，**main 也是实例**——`build_agent` 启动时 `registry.instances.register_main()`；任务线 `/session <名>` 切线即 spawn 成实例（`instances.rs::spawn_task_instance`，幂等：fork 原语 + 实例 MEMORY 段 + bound_path 对齐 + 6000 字符回灌）。各 channel 每条消息动态 `registry.instances.attached(<channel>)` 取当前实例的 agent（不再缓存 run() 时的 Arc）；`/session` 家族由 `slash.rs::try_session_command` 在 slash 分发**前**拦截（持当前实例锁时 try_lock 必失败，拦截点必须在锁前）切线 = idle 检查 + 换绑 + 旧实例 dormantize。**busy 三态判定** `InstanceHandle::busy_reason()`：agent 锁被持 → 跑回合中；gate 有 question → 等回答；有 approval → 等审批。实例 MEMORY 写路由与 home 只读守卫经 `ApprovalContext` 携带 `instance_memory_path` / `forbidden_home` 在 runner 拦截（`ToolRegistry` 被 fork 共享，不能按工具构造区分实例）；fork 改持独立 `ApprovalGate`（否则 main 的 pending 污染实例 idle 判定）。WebUI 是唯一并行入口，见下节。
...
| instances/  | 目录（任务实例） | `<实例名>/MEMORY.md` 实例私有记忆（ADR-0032 T5，runner 按 `instance_memory_path` 路由写入） |
...
> **First-run Bootstrap（`src/agent/bootstrap.rs`）**：`init` 落盘的 SOUL/USER 是占位符模板，全新 agent 既不知道用户是谁、也没有任何提示让它去问——画像文件因此永远空白。对策：回合起点（与 reminder 共用**同一次**读盘）判定两份文件是否"仍未填写"（`memory::is_unfilled`：内容为空或 `trim()` 后等于模板常量，纯字符串比较、不需要哈希），成立则注入一条 `[bootstrap]` 指令（`Context.bootstrap`，排在 reminder 之后成为最后一条消息），让 agent 先办用户交代的事、把 ≤5 个问题附在回复末尾，答案到手后用 `file_edit` 写回相对路径 SOUL.md / USER.md。要点：**零新状态、自我终止**——写盘即指纹改变、提示消失，不需要配置项/斜杠命令/"已引导过"标志位，用户拒答时按指令写一行说明同样彻底消解；**不进 system 前缀**（`system_prompt_base` 由 `init_system_meta` 缓存、逐轮字节一致是 KV 缓存前提）；**仅注册在主 agent**（子 agent 的 USER.md 由主 agent 复制，不该脱离主会话问用户）。写盘通路无需扩作用域：启动时 `workspace_root == workspace`（家目录），仅用户先 `/move` 才越界，且 `/move` 不持久化、重启自愈（指令里给出 `/move home` 降级指引）。详见 [docs/plans/2026-09-04-first-run-bootstrap.md](docs/plans/2026-09-04-first-run-bootstrap.md)。

> **Tail Reminder 自动生成（P6，`src/agent/reminder.rs`）**：长会话风格漂移的根因是 LLM 自我模仿 + 中段注意力稀释（SOUL/USER 结构上每轮完整重发，并未丢失）。对策：回合起点对 SOUL+USER 求 md5，与 `workspace/reminder.md` 记录的 hash 失配（或缺失）时**后台隔离 turn** 让 LLM 提炼 ≤120 token 的行为指令清单（走 compact\_provider 回退主模型），写盘后下一轮作为请求尾部消息注入（`Context.reminder`，排在状态栏/todo/env/task 之后、bootstrap 指令之前）。门禁：旧条件是"任一文件非空"，而模板文本永远非空会让全新 agent 第一回合白烧一个隔离 turn——现改为两份画像**都**仍是模板/空时不生成（`bootstrap::should_generate_reminder`）。MEMORY/skills 不参与 hash（避免 memory\_write 频繁重生成）；生成失败静默降级；文件头注释声明勿手改。
...
- **会话线（ADR-0031，2026-09-07 修订）**：通用线（`sessions.kind='main'`）之外可显式开任务线（`kind='task'`，`bound_path`= 该线**当前**所在目录，last-writer 纯元数据，由 /move 维护）——`/session <名>` 进出（旧名 `/task` `/tasks` 保留为前转发别名）、`/sessions` 列表、`/session close` 归档（`state='archived'` 不可续写）；切线时回灌目标线 sqlite 尾部（6000 字符预算，只取 user/assistant 正文），线名/目录/作用域失配经 Runtime Context（`Context.task_state`）注入。**WebUI 左侧栏（2026-09-23 起为实例 rail，取代旧 LINES 切线栏）**：chat 左侧 `INSTANCES` 栏由 `GET /api/instances` 驱动（活跃实例 + dormant 任务线合成，busy 用 try_lock 不挂），点击 `switchInstance` 切面板——多实例并行 turn（每连接 per-instance turns 表），事件经 `WebEvent::Instance` 封装分桶（main 不封装向后兼容）；dormant 线点击发 `open` 帧按需 spawn（首次 chat 帧也隐式 spawn，幂等）；`POST /api/session-lines/switch` 端点保留但前端不再使用。**web 的 `/session` 文本命令被拦截为 Side 提示**（切线入口 = 面板切换，CLI/IM 频道 `/session` 完整保留）。**重启双自愈**：`latest_session()` 仅回 kind='main'（任务线不进隐式续接，续做走显式 /session）、cwd 回家目录；`/move` 在线上自动改写 bound_path（notice 回显 was），两者对应关系靠统一的 `[scope]` 状态行 + 软提示，不强制；`fork_for_isolated`（cron）的 workspace_root pin 家目录不随主线漂移。
...
- 流式输出：chat 主路径已流式——回合循环消费 `provider.chat_stream`（SSE，走 Generation Guard 的 `consume_stream_guarded` 框架）；非流式 `chat()` 只剩 **sidecar 单发调用**（压缩摘要 / reminder 提炼 / vision 图片描述 / `/btw` 侧问 / 记忆压缩，均不带工具）。
...
- `interpret_inline`（`approval` 默认 / `off`）：**解释器内联载荷闸门（T3，2026-09-07 grill 定案）**。命令黑名单与路径校验都作用于命令行字符串，`python -c "…"` / `node -e` 的真正文件操作发生在解释器内部、框架零感知（实测三层全绕过）。`path_guard.rs::is_inline_interpreter_command`（引号感知段切分）识别内联形态——解释器 + `-c`/`-e`/`-r`/`--eval`/`--exec`、heredoc/stdin 进解释器、裸解释器/shell 被管道喂入（`curl … | bash`）——命中即在 `approval_decision` 强制人审（`ApprovalContext.terminal_inline_gate`，源自 `interpret_inline != "off"`），**即使落在 workspace/受信目录内**。**Windows shell 包装同属 T3（2026-09-15 高危修复）**：`powershell`/`pwsh` 的 `-Command`/`-c`/`-EncodedCommand` 与 `cmd` 的 `/C`/`/K`（含 MSYS `//C`、flag 前置、裸 shell 吃 stdin）由 `is_windows_shell_inline` 判定——引号载荷（`powershell -Command "Remove-Item C:\…"`）是含空格复合 token，路径提取完全不可见，曾同时绕过 workspace 白名单与审批通知静默执行越界删除；`-File script.ps1` 与 `python script.py` 同理不拦。刻意只拦内联：跑脚本文件不拦（路径走校验、写入在 transcript 可审计）；yolo 档显式弃权不受约束；delegate 频道绕过（P2-a 既有性质）。T2 无特权账户部署规范（真正的进程级防线）见 [docs/guide/security-hardening.md](docs/guide/security-hardening.md)。
...
默认 state dir `~/.llaia/`，可用 `--config-dir` 或环境变量 `LLAIA_HOME` 覆盖（CLI 参数优先）：
