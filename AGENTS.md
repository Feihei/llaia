...
> **WebUI Doctor（P6）**：`commands::doctor_checks`（结构化版 CLI doctor）：provider 连通性（openai\_compatible 才探 `/models`，5s 超时）、主模型链、context\_size 探测、`.env` 存在性+Unix 权限位、sessions.db、cron/mcp 解析、skills 计数。WebUI Config 页 Doctor section 分色展示。
