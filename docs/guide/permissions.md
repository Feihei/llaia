...
批准过的目录会进入**持久受信列表**（`<config_dir>/trusted_dirs.json`）：

- 列表内目录与当前 workspace 同等参与「是否在 workspace 内」判定——这些目录里的 `file_*` / `terminal` 操作**免审批**，逃出全部范围的仍需审批；
- **跨重启保留**：serve 重启后受信记录仍在，同一个目录不必重新 `/move` + `/ok`（写盘失败时静默退化为会话级）；
- agent 家目录 workspace 始终在列表中——`/move` 切走后，home 内的操作（SOUL/USER/MEMORY、`uploads/` 等）依旧免审；
- 列表只增不减，没有过期机制；要收权直接编辑或删除该 JSON 文件后重启。
