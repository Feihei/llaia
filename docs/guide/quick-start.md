...
在 Web UI 或终端里直接发消息即可。agent 可以读/写文件、跑终端命令、抓网页、搜索（需配置 `[tools.search].provider` 对应的 key）、记长期记忆。

第一次对话它还会**主动认识你**：先办你真正交代的事，然后把几个问题（怎么称呼你、用什么语言回话、希望它是什么风格）附在回复末尾。你的回答由它自己写进 `workspace/USER.md` 与 `SOUL.md`，之后不再重复问；不想填直接拒答就行。详见 [记忆与上下文](memory-and-context.md)。
