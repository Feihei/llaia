...
- **agent 自管（ADR-0027）**：agent 可直接创建 / 修改 skill，无需你手动写文件。
  ...
  - `skill_edit { name, content | patch, scope? }`：改已存在 skill。`content` 整文件替换；`patch` 为字符串（追加到正文）或对象 `{ "find": "...", "replace": "..." }`（单次精确替换）。兼容：部分本地模型会把该对象二次序列化成字符串发出，工具检测到整个字符串恰好是 `{find, replace}` 对象时会按替换处理而不是原文追加。
