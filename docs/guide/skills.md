...
- **agent 自管（ADR-0027）**：agent 可直接创建 / 修改 skill，无需你手动写文件。
  ...
  - `skill_edit { name, content | old_string + new_string | append, scope? }`：改已存在 skill，三种模式互斥单选：`content` 整文件替换；`old_string` + `new_string` 单次精确替换（须在 SKILL.md 中唯一命中，与 `file_edit` 同模型，`new_string` 可为空表示删除）；`append` 追加到正文末尾（保留 frontmatter）。
