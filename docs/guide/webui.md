...
- **会话历史**（Sessions 页）：浏览 / 导出 / 删除会话，Manage 模式可勾选删除单条消息；选中某条会话后，头部 **Archive older [N]d** 把**该会话** N 天前的消息整批搬进一条专属 archived 接收线（channel=`archive:<源uuid>`，同源重复点击复用同一条）——源线 id 不变、继续当活跃线写（不起新线、不回灌、live context 零感知），持续膨胀的 cron 长线也能原地瘦身。接收线在左栏可见（[archived] 徽标），可浏览 / 导出核对，点 Delete 整线删除即组合出「批量删除 N 天前记录」的效果。
