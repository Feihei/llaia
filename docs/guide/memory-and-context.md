...
模板本身有两个默认值要知道：`SOUL.md` 开头带 `# Name`，默认 **LLAIA**（旧版模板没这个名字，agent 被问到"你是谁"时只能现编一个）；`USER.md` 的语言偏好**留空**，由 agent 在引导时问你——旧模板预填的 `- language: Chinese` 会替你做主，对英文用户是错的。升级旧安装不需要动手：启动时会自动把**逐字节等于旧模板**的占位文件换成新版（主 workspace 和子 agent 目录都会过）；你哪怕只改过一行，它就不动——那说明已经是你的文件，不是占位符了。
