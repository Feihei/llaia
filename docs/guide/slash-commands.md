...
这是跨频道一致的审批流，详见 [权限与安全](permissions.md)。QQ 与 WebUI 另有图形入口，**两者都只是把这两条命令变成一次点击**（同一 `ApprovalGate`、同一续跑通路），`/ok` `/deny` 始终是兜底：

- QQ：审批提示消息上的「✅ 通过 / ❌ 拒绝」内联按钮（keyboard 被平台拒绝时自动降级回文本命令）。
- WebUI：聊天流内的**审批卡片**（工具名 + 范围 + 参数摘要 + 批准/拒绝按钮）。页面刷新或换设备后卡片靠 `GET /api/approvals` 补回；已在别处解析的卡片会标记为已失效。
