...
状态：**已实施**（2026-09-07，方案 2 骨架 + 方案 4 提示；rename-only + 双自愈 + 软提示，无锚概念、无作用域持久化；落地记录见 ADR-0031 修订节与 CHANGELOG v0.5.0）
日期：2026-09-07
...
6. `/new` → ~~维持现状（同线清上下文）~~ **2026-09-11 重新定案：取消 `/new`**。原文与代码不符（`c244f70` 起 `/new` 即真建新会话），且「新建 main 行」与「主线恒一条」方向矛盾。「换一页」改由 `/archive N` + `/clear` 配对承担，`/clear` 同步清 todo；见 [CHANGELOG.md](../CHANGELOG.md) §v0.5.0「session：会话线模型收口」与 ADR-0031 定案节。
