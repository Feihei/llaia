...
- **file_edit**：`old_string not found` 从「裸报错」变成「能自我纠正的报错」，并补上 Windows 的行尾陷阱（plan.md H1）。① 匹配曾是字节级 `content.matches(old)` 且零归一化，而仓库 `core.autocrlf = true` 又无 `.gitattributes`——新克隆到 Windows 得到 CRLF 工作树，模型按 LF 复制 `file_read` 的内容必然 0 命中，工具直接不可用。新增 `match_with_eol_tolerance`：首选完全一致，未命中且两侧行尾单纯不同时换算行尾重试，写回只替换命中片段、不整篇改行尾（混合行尾不纳入宽容）。② 新增 `not_found_hint`：0 命中时以 `old_string` 里首个可命中行为锚点，附一段带 1 起始行号的文件摘录（上限 24 行 / 1000 字符），无锚点则明确提示「内容可能过期，重新 `file_read` 后逐字复制」——此前只回一句 not found，模型会抱着同一份脑内快照连撞四五次。触发排查的四个真实案例全是模型侧文本不符（判定本身没错），但工具当时帮不上忙。3 条单测
- **tool_call**：标签降级模式下模型把工具调用包在 `` ```json `` 围栏里会被**静默丢弃**（实测于 first-run bootstrap：agent 试图写 SOUL/USER，用户侧什么都没发生）——`stream_parser.rs` 的 `FENCE_LANGS` 只认 `tool_call|toolcall|tool-call|invoke`，`json` 不匹配就整块按普通文本透传。新增 `GENERIC_FENCE_LANGS = ["json"]`：通用语言名的块走**严格判定**（`value_to_fenced_call` 要求同时带 `arguments` 键），否则 `{"name":"张三","age":3}` 这类展示给用户的数据会被凭空执行成一次 unknown tool 调用；专用围栏 ```` ```tool_call ```` 维持原宽松判定，零行为回归。另补两处还原：解析失败时连开栏原文一起吐（原来只剩丢格式的正文 + 一个孤儿闭栏），未闭合的通用围栏在 `finish()` 还原为文本而非静默吞掉（截断时宁可让用户看到半截 JSON），未闭合的专用围栏仍按原语义丢弃防泄漏。代价如实记录：```` ```json ```` 块要缓冲到闭栏才输出，不再逐字流式（```` ```python ```` 等不受影响，`test_java_fence_not_hijacked_by_json_prefix` 钉住同前缀不被劫持）。顺带删除 `tool_call/tag_parser.rs`——同一套 tag+fence 规则的正则版实现，334 行、**零生产调用点**（改它等于没改），等价覆盖落在 `stream_parser.rs` 的 7 条新单测，`agent/mod.rs` 另加一条端到端用例证明围栏调用真被执行
...
- **path_guard**：terminal 误判 Git Bash 风格路径越界、workspace 内命令每次弹审批（实测于 cottage 会话：agent 从 Git Bash 产出 `cd /c/Users/THAD/.llaia/workspace/cottage && cp ...`，审批提示报 `terminal (outside workspace)`）。根因：terminal 在 Windows 上经 Git Bash 执行，agent 产出 MSYS 风格绝对路径 `/c/...`，而 Windows 的 `PathBuf` 认为它无盘符前缀，与 `C:\Users\...` 形态的 workspace 根做 `starts_with` 比较必然失败——白名单校验与审批判定两层一致地误判，命令本身却完全能跑。修复：新增 `path_guard::convert_msys_path()`，把**单字母首段**（盘符缩写）的 `/c/Users/x` 换算成 `C:\Users\x`（含分隔符统一），在 `validate_path` 与 `resolve_approved_path` 入口、危险黑名单检查**之前**应用（`/c/Windows` 同样命中黑名单，防线不因换算松脱）；`/usr`、`/etc` 等多字符首段的 Unix 路径原样放行不误换算。审批层（`approval_decision`）与执行层（file_* / delegate / web send）共用这两个入口函数，判定自动一致。3 条 Windows-only 回归测试（MSYS 路径落 workspace、单字母段才换算、`cd /c/... && cp ...` 整条命令免审批）
- **webui**：WebUI 发出去的图**从来没进过模型**（实测于 cottage 会话：落库的 user 消息长这样——`这是你刚弄的小茅屋样子[invalid path: path escapes base: uploads/xxx_cattage.jpg]`，用户因此看到 agent 改用 Python PIL 猜像素）。根因是三段首尾相接的错位：`web/mod.rs::upload` 回传的相对路径**自带** `uploads/` 前缀 → `app.js` 原样回传 → `channels/web.rs::build_user_message` 又把 base 设成 `workspace/uploads`，于是解析出 `uploads/uploads/xxx` 这个永远不存在的路径，图片静默退化成一段文本占位符。CLI 频道同一逻辑用的是 base=`workspace`，所以**只有 WebUI 坏**，且自 `2da26e8`（WS handler 落地那次）诞生即坏、没有回归点。修复按因/果两刀：① base 改回 `workspace` 与 CLI 对齐（`WebChannel.workspace` 是固定家目录、不随 `/move` 漂移），新增 `upload_rel` 兼容「带前缀」与「裸文件名」两种形态，中间的 `..` 原样透传给 `resolve_within` 的门禁；② `resolve_within` 曾对 base 与 joined **各自独立** `canonicalize` 并在失败时回退原值，而 Windows 的 canonicalize 返回带 verbatim 前缀的路径——一侧 canonical、一侧原始时 `starts_with` 恒假，**把「路径拼错」报成了「越权逃逸」**，安全措辞把排障直接带偏（本人第一反应也是去查 path_guard）。新增 `canonicalize_lenient`（逐级向上对最近的存在祖先 canonicalize、再拼回余下分量）使两侧形态一致，不存在改由调用方诚实报 `image load failed`。顺带澄清两处既有叙事：guide「上传的文件可在对话里引用」与历史清单「多媒体上传/读取 ✅」此前只覆盖了落盘与取回，入模这一环是断的；配置侧本无障碍（未配 `vision_model` 即走主模型多模态桥接，`openai_compat` 的 `MessageContent::Multimodal` 序列化正常）。6 条新单测，其中 `attaches_prefixed_upload` 做过**反向验证**（临时还原旧 base 后该测试确实 FAILED，钉住它不是空转）；traversal / absolute / windows-drive 三条安全测试继续通过，宽容 canonicalize 未放松边界
- **webui**：WS 断开时发消息**静默无响应**（上一条修复之后才暴露：用户重编译重启 serve、浏览器页面还开着，之后发图「没反应、点 Stop 也没反应」，全程无任何报错）。取证：`uploads/` 里两张图 18:02 / 18:07 确实落盘（HTTP 通路正常），但 `web received message`（打在 busy 判断之后、`build_user_message` 之前）一次都没出现 → 帧根本没进后端，与上一条服务端修复无关。根因在前端：`send()` / `stop()` 直接 `this.ws.send(...)` 不查 `readyState`（同文件的心跳 ping 反倒查了），socket 非 OPEN 时抛 `InvalidStateError`，异常被 Alpine 的点击处理器吞掉；而 `busy = true` 与本地消息 push 都发生在 send **之前**，于是界面停在 Steer 态、输入与图片看似已发出实则什么都没送，再点 Stop 又抛一次——整体假死且零提示。修复：新增 `wsOpen()` 就绪守卫，`busy` 移到真正发出之后才置位；`onclose` 落下 `busy` 并提示 `[disconnected]`（连接断了不会再有事件回流，留着 true 就是骗界面）。**首版只提示不排队，实测立刻暴露新问题**：用户第一次 Send 撞上「页面刚加载、socket 还在握手」的窗口被拒，原样再按一次才发出去——把竞态转嫁给了用户。改为 `outbox` 排队：未就绪时把帧（chat / steer / stop 一视同仁）按序存下、气泡与缩略图照常立刻上屏、提示 `[queued] …`，`onopen` 与 `auth_ok` 两处 `flushOutbox()` 补发（补发成功要把 busy 立回来，否则按钮停在 Send 态、Stop 禁用，用户没法中断自己刚补发的消息）；运行中 steer 分支顺带补齐：帧只带文本，故**图片留在芯片里不静默丢弃**并给出说明。`onUpload` 清空 `input.value`，否则连续选同一张图不再触发 change。逻辑用 node 直接驱动真实 `llaiaApp()` 验过（无就绪 socket 时任何 `send` 调用都会抛，测试里让桩函数抛错以证明守卫确实生效）另把服务端三处图片降级（invalid path / not an image / image load failed）收口成一条 `WARN`：这条通路静默降级了太久，上一次只能靠 agent 改用 PIL 反推出「图丢了」
- **webui**：chat 右侧状态栏**常驻**。推翻同日刚交付的两栏布局决策（那条 Features 里的「侧栏带 `x-show="todos.length || env"`，两块都空时整体隐藏不留白」）——用户实测反馈「右侧栏又没了」，而这不是一次性故障是设计陷阱：TODO 只有当前会话用过 `todo` 工具才有条目，于是整条栏的可见性实际押在 ENV 一块上；而 ENV 一旦为空（探测未跑 / 拉取失败），**唯一的恢复入口 Refresh 按钮跟着整块一起被藏掉**，用户既看不到栏也没有点一下就能重探的地方，只能刷新页面赌一次。更糟的是 `loadEnv` / `refreshEnv` 的 `catch` 是「非致命：UI 静默跳过」，为空时不留任何痕迹。改为：aside 不再条件渲染；TODO 块仍只在有清单时占位（这块没有恢复入口被藏的问题）；ENV 块常驻，探测串为空时显示 `envError` 或「not probed yet — press Refresh」，两个 `catch` 不再吞异常而是把 HTTP 状态与异常写进 `envError`。验证走无头 Edge `--dump-dom` 看渲染后 DOM：`x-show="env"` 项 `style=""`（可见且含真实探测串）、`x-show="!env"` 项 `display:none`（提示正确让位）、`x-show="todos.length"` 项 `display:none`（无清单不占位）、aside 不再被写 display

**Features**
- **webui**：chat 左侧**活跃会话线列表**——把 ADR-0031 的任务线从「命令行回忆」提到「一眼可见、一键可达」（WebUI 侧通路）。此前切线只能敲 `/session <名>`，有哪几条线、各自绑着哪个目录全靠 `/sessions` 文本列表脑补。chat 布局改三栏：左侧 180px `session-rail` 里主线 `⌂ Main` 固定置顶（点击回主线 = 恢复家目录 scope，对齐 `/move home` 语义），任务线显示线名 + bound_path/channel（hover 显完整路径），当前线绿色高亮；无任务线时提示 `/session <name>` 用法。列表 15s 轮询（turn 运行中跳过，防 agent 锁挂请求），回合结束 / 点 Chat tab / Refresh 按钮即时刷新，回合内用 `/session` 新建的线下一拍就出现。切换走 `POST /api/session-lines/switch`，核心逻辑收在 `slash.rs::switch_line_for_web`（pub、返回 `WebLineSwitch{notice, backfill}`）：完整复用 `/session` 的 `switch_session` / `switch_to_main` 上下文回灌通路，**差异点是 bound_dir 跟随**——点击是显式用户意图，切到任务线时同步把 `workspace_root` 切到该线 bound_path（该目录此前必经 `/move` 审批才落库，不再二次审批；目录已消失仍切但 notice 带 warning），回主线恢复家目录。响应带回目标线尾部 user/assistant 消息，前端清空本地流后直接渲染成对话气泡 + notice 工具条——**后端回灌进 context 的和屏幕上看到的严格同一批**。`GET /api/session-lines` 与 `latest_main_session` 同一套过滤（kind='main' + 非 cron + 非归档；任务线 `list_open_tasks` 再滤 `cron:` 前缀）。≤900px 窄屏左侧栏折到消息流上方限高可滚。测试 +2（主线/任务线互切回灌与幂等报错；tempdir 验证 bound_dir 真实切走/恢复家目录）
- **thinking**：思考能力模型三段交付（[plan](plans/2026-09-10-thinking-capability-model.md)，五家在用端点 wire 实测驱动）。**P0 留存**：`StreamEvent::ReasoningDelta` 收集 `reasoning_content`/`thinking` SSE 字段 → `ChatMessage.reasoning_content` → sqlite 五列写入（`append_message_with_reasoning`，终结「列建了但恒 NULL」）→ WebUI 会话详情折叠思考块；`reasoning_to_content` 折回通路不双份存储；预算计入、`cheap_normalize` 不碰思考逐字、`drop_oldest_unit` 随消息整条丢。**P1 档位与门禁**：`ChatRequest.disable_thinking: bool` 升级为 `thinking: Option<ThinkingIntent>`（auto/none/low/medium/high/max），`[provider.<id>.<model>.thinking]` 能力声明（`level_wire`/`off_wire` 方言 × `preserve`）+ `resolve_thinking()` provider 层唯一收口；`/reasoning` 保名改义（on/off 作别名），按能力三态回显、`none` 落在不支持的端点**拒收报错不静默降级**；生效态每回合尾部注入；guard 重试按 `off_wire` 分流（unknown/unsupported 不再假装关思考——终结 Ollama「思考吃满 max_tokens → 重试无效」死循环）；`finish_reason=length` + 空内容改判**输出预算耗尽**（不重试不计熔断，诊断指向 `--reasoning-budget`）；sidecar 单发统一 Off 意图。**P2 回传**：`preserve=true` 时历史 assistant 思考逐字随请求发回（默认关；远端 b10645 丢弃、Ollama 不消费均有控制组实测，llama.cpp 新 build `--reasoning-preserve` 默认开）；WebUI Config 模型行 thinking 折叠面板（值必须来自实测，全空 = 不写段）。明确否决：pi-ai 式 7×11 全矩阵、按模型名 substring 猜方言（方言挂部署粒度——同名 deepseek-v4-flash 跨网关分裂、llama.cpp 跨家族稳定）、云端启动期探测、`/think` 新命令。测试 +25（能力矩阵、wire 正反向、guard 分流、预算 vs 退化、tail 注入、serde skip 方向、preserve 逐字红线）。**P0 追加：chat 界面实时思考折叠块**——此前思考只在会话详情可见，直播对话没有思考面。`TurnEvent::Reasoning` 旁路帧：`consume_stream_guarded` 在收集留存的同一处把 `ReasoningDelta` 转发给频道，与可见文本的 Chunk **彻底分流**（思考永不进正文）；`OutputSink.on_reasoning` 默认空实现，CLI 与各 IM 频道零影响，仅 WebSink 转 `reasoning` WS 帧。前端把思考累积到流式中的 assistant 消息，渲染成与会话详情同族的紫调折叠块：流式期间自动展开（`Thinking… N chars`），done/error/interrupted 自动收起，手动点开/收起后以手动为准；测试钉住「Reasoning 帧完整到达且思考文本绝不漏进可见 Chunk」
- **terminal**：命令行路径提取重写——引号感知 + flag 语义（plan.md S1，2026-09-07 grill 定案，定位是**误报修复**而非安全防线）。旧实现 `split_whitespace` 暴力抠 token 不懂引号、不识 flag：`git commit -m "fix: a/b"` 的提交消息、`sed 's/foo/bar/g'` 的脚本、`grep "x/y"` 的模式、`find -path "*/t/*"` 的 glob、`git clone https://…` 的 URL、`HTTP_PROXY=http://…` 的赋值，只要含 `/` 都会被抠成假路径送去范围校验而误拒（#C 同族根因）。重写 `extract_path_tokens`：① 引号感知 tokenizer（引号内空白不切分、引号剥除——带引号的真实路径此前连引号送去校验必败，安全顺带增强；引号外 `\` 仅在空白/引号前转义，Windows 盘符/UNC 不被破坏）；② flag 语义表（`-m`/`-e`/`-r` + message/eval/regex/name/path/include/format/data/url 等非路径值跳过，含内联 `--flag=value`；未知 flag 保持默认判定，宁可多查不漏检）；③ 位置语义（grep/rg/sed/awk/printf 首裸参、echo 全部裸参不按路径判定）；④ `VAR=value` 赋值按值判定，URL 跳过但 `VAR=/real/path` 仍检出。安全不回退：`-C /outside` 等未入表 flag 照常检出（tar 换目录向量有钉住测试）。审批层与执行层共用同一提取函数，行为自动一致。5 条新单测 + 既有提取回归全过（lib 576）
- **terminal**：解释器内联载荷强制人审（plan.md T3，2026-09-07 grill 定案）——命令黑名单与路径校验都作用于命令行字符串本身，而 `python -c "…"` / `node -e` 的真正文件操作发生在解释器内部、框架零感知（实测命令黑名单、shell 包装拦截、路径校验三层全绕过）。新增 `path_guard.rs::is_inline_interpreter_command`（引号感知段切分，按顶层 `|`/`;`/`&&`/换行切段）：识别解释器 + 内联 flag（`-c`/`-e`/`-r`/`--eval`/`--exec`）、heredoc / `-` 显式 stdin 进解释器、`deno eval` 子命令、裸解释器/shell 被管道喂入（`echo x | python`、经典 `curl … | bash`）；段首 `VAR=value` 前缀与带路径/`.exe` 后缀的解释器均可识别。命中即在 `approval_decision` 强制 `/ok` 人审（`ApprovalContext.terminal_inline_gate`，新配置 `[tools.terminal].interpret_inline`，默认 `approval`，`off` 关闭），**即使落在 workspace / 受信目录内**。刻意只拦内联：跑脚本文件（`python script.py`）不拦——脚本路径仍走 path 校验、写入动作在 transcript 可审计；`python -m`、非解释器命令的同名 flag（`grep -c`）不误伤；yolo 档显式弃权不受约束、delegate 频道维持 P2-a 既有绕过（注释留档）。同批交付 T2 文档化：新增 [docs/guide/security-hardening.md](guide/security-hardening.md)（无特权账户部署规范 + T1 OS 沙箱不采纳的评估记录）。16 条检测单测 + 3 条审批闸门单测
- **terminal**：脚本绕过防护专项收口（旧 plan.md P7，grill 2026-09-07 定案）——除上述 T3 / S1 / T2 三项交付外，同批拍板的**不做项**留档：**S2 `sh -c` 载荷递归解析划掉**（核实 `check_shell_wrappers` 对 `bash/sh/zsh/fish + -c` 是硬拒绝而非人审，不存在「套壳漏网需要解析」的安全缺口；剩余价值仅是硬拒改解析放行的易用性，单用户 agent 几乎不需要 `sh -c`（`&&` 链即可），不值得引入「解析器认为安全」的新信任面）；**T1 OS 级沙箱预判不做**（与「轻量、可移植、单 crate」产品定位正面冲突，评估记录见 [docs/guide/security-hardening.md](guide/security-hardening.md)，真实发生事故再重启评估）；**T4 缩小爆炸半径全部不做**（git 跟踪/备份指引、read-only 权限档、断网三项均不留，整体归档；已知代价：误删场景无兜底叙事，接受）。旧 P7 编号（T1–T4 / S1–S2）随之退役，plan.md 的 P7 重新立项
...
- **session**：`/task` 改名 `/session`（旧名 `/task` `/tasks` 保留为纯转发别名，`kind='task'` 内部值不动），配套三件套落地 9/6 事故取证的结论（ADR-0031 修订节，完整设计与 goose/pi/jcode/DSH/opencode 对比见 plans/2026-09-07-task-to-session.md）。① **重启双自愈**：`latest_session()` 仅回 `kind='main'`——任务线不再被隐式续接（空历史+任务身份曾诱导模型脑补上下文），续做走显式 `/session <名>` 自动回灌尾部；cwd 侧维持回家目录。② **bound_path 语义升级**：从「创建时目录」改为「该线**当前**所在目录」（last-writer、仍是纯元数据不参与审批），由 `/move` 接管——线上批准即 `set_bound_path`（新方法）改写并回显 `(was X)`，回 home 解绑，主线不写库；不引入 opencode 式 project 锚。③ **对应性软提示**：`/session` 切线与 `/move` 落地的 notice 统一追加 `[scope]` 状态行（失配给对齐 tip、无绑定给即绑定 tip、主线 /move 命中他线绑定给切线 tip），`Context.task_state` 注入同步失配句——框架摆事实、同步留给用户。顺带修同源缺口：`fork_for_isolated`（cron/委派）不再共享主线 `workspace_root` Arc，改 pin 家目录独立副本。新增/改造单测：别名冒烟、scope 行、`set_bound_path` 换绑解绑主线无效、latest_session 不续任务线、fork 作用域隔离
- **session**：会话线模型收口（[plan.md](plan.md)「会话线模型收口」、ADR-0031 定案节，2026-09-11）——`/new` **移除**、新增 `/archive [days]`、`/clear` 行为扩展，主线恒为一条。① **`/new` 删除**：它在"主线恒一条"模型下反成矛盾源——每按一次造一条 `latest_session` 永不再选中的僵尸 main 行，而 WebUI Sessions 列表恒可见（`list_sessions` 无 kind 过滤）；"唯一能造 main 线的命令"的顾虑由既有兜底消解（启动 `latest_session()` 为 None 即建、`switch_to_main` fallback），全新安装与主线被整线删除两个场景都有落点。② **`/archive [days]`**：与 WebUI「Archive older」同一条 `archive_messages_before` 通路——把当前线 N 天前（默认 30、钳制 1–3650）的消息搬进归档桶，源线继续当活跃线用、live context 零感知，任务线上同样成立；归档桶隔离性现成继承（`archive:` 前缀 channel + archived + kind='task' 三重过滤）。**桶标题改动态更新**：每次归档后重写为桶内最新一条消息的日期（`archive of <名> (through <日期>)`），替代首写死 cutoff——旧实现重复归档累加进同桶而标签不动，装着 09-20 的消息、写着 09-09（本次顺手修复，WebUI 通路同样受益）。③ **`/archive` 与 `/clear` 职责正交、不绑定**：前者纯存储层，后者清内存上下文；「换一页并立即遗忘」= 两者配对，由用户自行组合，不设 `/reset` 聚合命令（曾拟议、经 grill 否决）。N=0 的同秒竞态不修（`created_at < now` 天然排除同秒末尾，N≥1 是主用例）。④ **`/clear` 连带清 todo**：`TodoStore::clear_current()`（内存置空 + 删 `todos/<uuid>.json`）——todo 定位短期小计划，主线 uuid 恒定、不随清则清单永生，长大计划本就该落文件。测试：`clear_current` 两态（清空 + 无 session inert）、归档桶动态标题断言、`/archive` 参数边界与 no-op、`/clear` 集成。文档同步：AGENTS.md 会话模型与命令清单、guide 三处、glossary、ADR-0024/0031、task-to-session plan 处置 6 加注（原文与代码不符的遗留一并修正）
...
- **session（移除）**：压缩时自动生成会话标题功能**移除**（2026-09-08 应用户要求，issues.md #3）——「自动总结的主题」辨识度低，且每次压缩多一次 sidecar LLM 调用。删除 `ensure_session_title` 与 `/compact` / 自动压缩里的调用、`sanitize_title` / `cap_chars` 辅助、sqlite 的 `session_title` / `set_session_title`。`sessions.title` 列保留：任务线（ADR-0031）的任务名即 title，WebUI 会话列表展示逻辑不动
- **webui**：会话历史卫生工具「Archive older [N]d」（Sessions 页某条会话的头部，per-session）——sessions.db 归档层只增不减，且持续膨胀的恰是**天天在写的活跃线**（morning_news cron 已 210 万字符），按线闲置批量处置够不到它们，既有通路只有整线 Delete 与 Manage 勾选逐条删。新增 `SessionStore::archive_messages_before(src, cutoff)`：单事务把该线 `created_at` 早于截止的消息整批**搬进**专属接收线（channel=`archive:<源uuid>`、state='archived'、kind='task'，同源重复点击复用同一条、0 命中不开线）+ `POST /api/sessions/:uuid/archive-older?days=N`（clamp 1..=3650）+ 头部天数输入与按钮。**源线 id 不变、继续当活跃线写**：不起新线、不回灌、live context 零感知，cron 长线原地瘦身。接收线三重隔离（`session_by_channel` 精确匹配永不再命中 `archive:` 前缀线；`latest_session` / `find_open_task` 被 kind+state 双排除），在左栏可浏览/导出核对，整线 Delete 组合出「批量删除 N 天前记录」。FTS 按 rowid 对齐 `messages.id`、tool\_calls 挂 `message_id`，搬运不断链（单测钉住原 id 与顺序保持）；接收线自身拒绝再归档（400）。同批推翻前一日首版「按线 last\_activity 整线批量归档」——够不到活跃线且会误伤仍在续用的任务线（`/session <名>` 找不到 archived 线）。单测：搬运命中数、id/顺序保持、源线 latest\_session 归属不变、0 命中零副作用、复用接收线不建第二桶
- **webui**：turn 运行中的 Steer 按钮与可移除上传芯片。① 旧版输入框 / Send 按钮绑 `:disabled="busy"`，turn 一跑起来就输入不了——而后端 `web.rs` 本就支持运行中收 `/steer`（插话队列）与 `type:"stop"` WS 消息，通路纯被前端堵死（审批等待期不受影响：deferred turn 已发 Done，输入框恢复）。改为输入框保持可用、Send 按钮运行中变 **Steer**（`x-text` 切换），点击 / Enter 把输入自动加 `/steer ` 前缀投递（后端只认 `split_steer` 前缀，纯文本由前端包装），`/stop` 文本仍走中断。末轮残留语义不变：非末轮迭代顶部 drain 注入，正常收尾 / force_summary 的残留 steer 由后端丢弃并回显 `[steer not applied]` 提示（plan.md #I ③ 既定设计，「落不了地的插话不如明确拒收」）。② 待传图片从「composer 内嵌一行完整路径」改为独立芯片行：缩略图 + 文件名（hover 显完整路径）+ × 移除按钮（`removeUpload`），不再挤占输入行
- **webui**：chat 界面两栏布局——todo 清单与 env 探测从底栏挪到右侧状态栏。chat-pane 包一层 `chat-layout`：左侧 `chat-main`（消息流 + composer + QUESTIONS 面板），右侧 280px `side-panel`（TODO + ENV，ENV 的 Refresh 保留）；侧栏带 `x-show="todos.length || env"`，两块都空时整体隐藏不留白。面板复用既有 `.todo-panel` 样式（绿/黄标识不变），仅侧栏内调外边距；≤900px 窄屏回退为侧栏折到消息流下方、限高 240px 可滚动
- **webui**：chat 内 tool 结果可折叠（与 Sessions 页同款交互）——旧版 `tool_start` / `tool_result` 是两条独立消息：工具名一行 + 输出整块平铺，长输出把聊天流撑出几屏。改为事件流合并（`tool_result` 回填最近 `tool_start` 消息，`{text: 工具名, output}`，结果未到先挂起），渲染为折叠条 `▸ <tool> — result (N chars)`，点击展开等宽 `<pre>` 正文（限高 320px 内部滚动）；无输出的 tool 消息（`[error]` / `[Interrupted]` / `[cron]` 通知、执行中挂起的工具名）保持原纯文本形态
...
- **provider**：模型级 `enabled` 开关（plans/2026-09-09-model-enabled-toggle.md）——需求是「模型参数先记在 `config.toml` 里，但暂时不想让它被选中」，此前只有两条脏路：删掉整个 model 表（参数丢失）或留着并在脑内排除（`/provider` 序号会数到它）。`ModelConfig` 加 `enabled: bool`（默认 true，存量零迁移）。**只管可发现性，不管可用性**：`flatten_model_refs`（`/provider` 列表兼 `<序号>` 索引基准）与前端 `modelRefs()`（agent model / fallback 下拉）过滤 disabled，但 `provider_from_ref` 刻意**不拦**——它是 agent init / fallback 链 / compact / vision / `/provider` 切换的唯一收口，在此硬拦会让「关掉当前 `agent.model` 指向的模型」变成下次启动即失败、且 WebUI 保存连改回来都走不通；下拉靠既有的 `xxx (current)` 兜底，零新代码。`skip_serializing_if` 的**方向是本次唯一的坑**：必须 true 时省略（只写显式 `= false`）；写成 false 时省略会让 disabled 不落盘，配合 provider 子树 replace 合并（缺失即删）在保存瞬间把关闭态洗回 true——症状是「关掉再打开页面又亮着」，且错在 serde 层、前后端都看不出异常，已用单测钉住双向断言。连带后果是 `skip_serializing_if` 对 `serde_json` 同样生效、`GET /api/config` 省略该键，前端装载必须 `enabled ??= true` 归一化，否则 checkbox 把启用态显示成未勾选（这才是 WebUI 与 config 真正会失同步之处）。间接引用收敛抽成 `Config::reconcile_disabled_models`：fallback 指向 disabled 的项剔除并 warn、`compact_model` / `vision_model` warn + 置 None 回退主模型（顺带补上 `vision_model` 此前完全没有的引用校验），**只改内存态、不改写盘**，故盘上引用原样保留、模型重新启用后自动恢复无需重填；由 `Config::load` 与 `put_config` 各调一次（后者不走 load，少调就要重启才生效）。开关必须用 checkbox 而非 select：`x-model` 在 checkbox 上产出真 boolean，`<select>` 给字符串 `"true"` 会让 serde bool 解析失败、整个 PUT 返 400。UI 采 astrbot 式：模型行常驻 + 灰化 + 开关（抹掉行会让关掉的模型找不回来）。doctor 的**探测**侧零改动——它只 per provider 发一个 `GET /models`、模型级动作仅 `detect_context_size`（且只探 main model），与 `enabled` 正交。但**输出侧补两处**，都是本改动自己制造的状态：CLI doctor 的 model 清单加 `[disabled]` 标记（它本就遍历所有 model 打印，不加会出现「doctor 列得出、`/provider` 列不出」的视图矛盾，像 bug）；`/provider` 在当前模型自身被隐藏时补一行说明（否则列表里连 `*` 都没有，像模型丢了）。6 条单测（缺省即 true、序列化方向双向断言、reconcile 三类引用、全启用不误伤、`/provider` 过滤、隐藏当前模型的提示）
- **cli**：`LLAIA_HOME` 环境变量 + `llaia serve --host/--port`（采纳「flag 面不扩、并行测试走 env」的方案）。`--config-dir` 保留原名不改：`--root` 更短但**词已被占用**——仓库里 `workspace_root` 有 50 处，指文件/终端工具的实时作用域且可被 `/move` 切换，AGENTS.md 还专门有一节讲「两个 `workspace` 的区别」；`--root E:/proj` 几乎必然被读成「把工作目录设成这儿」，恰是它**不**做的事。解析优先级 `--config-dir` > `LLAIA_HOME` > `~/.llaia`，抽成纯函数 `resolve_state_dir` 给三级各钉单测；**空串按未设置处理**（`LLAIA_HOME=` 在 CI 里比缺变量更常见，照收会把状态目录变成相对路径空串、把 config/sessions.db 静默写进 CWD）；三条都不通时返回明确错误，替掉原来的 `expect("no home dir")`（本仓库约定生产路径不 expect）。`--host/--port` 经 `effective_webui(file, bind)` 得到「绑定用」的拷贝，**只喂给 `WebChannel` 的绑定与日志**：`live_config`（Config 页 `GET/PUT /api/config` 读写的文件态）不被覆盖值污染——否则页面显示一个并非来自配置的端口，用户下一次保存就把临时端口静默固化成常驻配置（单测钉住「覆盖绝不改写文件态」）。覆盖生效时打一条 `webui bind overridden by CLI` INFO 摆出「文件值 vs 实际监听」，免得看着像 bug。实盘验过：`LLAIA_HOME=<临时目录> serve --port 51220` 只在该目录生成骨架、监听 51220、页面 200、51217 不通，而盘上 `config.toml` 与 `/api/config` 里仍是 51217。顺带修 glossary 里 `llaia init [--workspace <path>]` 的过时参数面（实际只有 `--force`）。7 条新单测（lib 675）
- **webui**：对话流里看得见**自己发的图**。用户的原话是「否则我都不知道我发了没」：待发图有芯片行，但 `send()` push 的 user 消息只带 `text`，图片随 `uploaded` 一起清空，服务端组装的多模态 part 也不回显，发完什么痕迹都没有。现在 user 消息挂 `images` 数组，气泡内渲染一排小缩略图（与芯片同源走 `/file?path=…`，点开看原图），`.msg-img` 限 160×120 刻意压过全局 `.msg img` 的 400px——「看得见」不等于把版面撑爆。散落在 HTML 各处的 URL 拼接表达式收进 `fileUrl()`
- **webui**：composer 附件入口改成输入框左侧的 **[+]** 按钮——原生 `input[type=file]` 会常显「Choose File + 文件名」，而文件名在下方待发芯片行里已经有了，重复且丑。改成隐藏原生 input、由 `+`（`$refs.fileInput.click()`）唤起选择器；`hidden` 之外再补一条 `display:none !important` 兜底，防任何主题样式把它放回来。全局 button 的 `[` `]` 装饰正好把加号包成 `[+]`，无需特例样式
- **webui**：Config 页各 section 的操作按钮集中到固定顶栏——此前 Save 埋在各 section 内容底部，长页面里点保存要先滚到底、保存反馈消息也看不见。结构上把 `config-head`（"❯ CONFIG xxx" 标题栏）从滚动容器 `.content` 里提出来，挂到新加的 `config-main` 列容器顶部，成为真固定顶栏、只有内容区滚动（比 sticky 补丁干净）。顶栏左侧紧跟标题、按 section 显示对应动作：runtime/log/provider/agent/channels/tools 的 `Save`（provider 另有 `+ Add provider`、agent 另有 `+ Add agent`）；raw 的 `Validate`+`Save`；cron tasks/history 的 `Refresh` 与 raw 的 `Save`；mcp servers 的 `Save`+`Refresh` 与 raw 的 `Save`；skills 列表的 `Refresh`+`+ New skill` 与编辑页的 `Save`+`Back`；doctor 的 `Run checks`（含 Running… 禁用态）；about 的 `Restart Service`+`Check Updates`+`Stop Service`。保存反馈消息（rawMsg/cronMsg/cronRawMsg/mcpMsg/mcpRawMsg/skillMsg/skillContentMsg/restartMsg/shutdownMsg）随按钮上移，点按钮处即可见结果；正文残留的 cron-toolbar 工具条与底部按钮全部清除，卡片/表格行内按钮（Edit/Delete/Trigger/Test/+ Add model）保持原位。两个样式坑：`#config-pane .btn-add { margin: 8px 0 }` 会把 provider/agent/skills 三页的标题栏撑得比别的高，`.config-head-actions` 内归零；`.config-head span` 后代选择器会把按钮内文字染灰，改直接子选择器 `.config-head > span`

**Refactor / 健壮性**
- **robustness**：主干体检 backlog 机械项提升（plan.md H4，2026-09-06 交付）。① 生产路径 `lock().unwrap()` 毒锁恢复：`slash.rs` 4 处 + `sqlite.rs` 36 处（立项时估「约 20 处」偏少，实数 40）全部机械替换为 `lock().unwrap_or_else(|e| e.into_inner())`——锁内均同步调用、无 await，`Mutex` 毒化后恢复守卫不丢正确性，与「生产路径不用 unwrap」约定对齐；② 常量正则 `unwrap()` 补「模式串编译期写死且合法，构造不可能失败」注释（`secrets.rs::is_plaintext`、`config.rs::expand_string`），`approval.rs` 核实为立项笔误——该文件没有常量正则，实际处理的是 `single_question` 里受 `len()==1` 保护的 `into_iter().next().unwrap()`，同批补注释
