...
- **webui**：会话历史卫生工具「Archive idle > N day(s)」（Sessions 页）——sessions.db 归档层只增不减（历史 cron 线、旧主线、废弃任务线），既有通路只有整线 Delete 与 Manage 勾选逐条删，缺批量处置。新增 `SessionStore::archive_sessions_before(cutoff)`（`last_activity` 早于截止的未归档线整线置 `state='archived'`）+ `POST /api/sessions/archive-older?days=N`（clamp 1..=3650）+ 侧栏天数输入与按钮。语义是**纯状态位翻转**：不轮换、不起新线、不删数据（`session_by_channel` 不加 state 过滤，cron 仍写同一条线），仍在写入的活跃线因写入刷新 `last_activity` 天然免疫；刻意不做「Delete archived」批量按钮——会误伤 `/session close` 手动归档、本应长期留档的任务线，要彻底清盘对归档线逐条点 Delete 即可组合出 delete-N-days-ago 的效果。单测：cutoff 命中、活跃线免疫、幂等重跑、归档线仍可续写
