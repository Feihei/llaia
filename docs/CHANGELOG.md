...
**Bug fixes / 稳定性**
- **todo**：每轮注入的 todo 清单此前**连 `[x]` 完成项和编号一起回灌**，弱模型无视状态标记、把陈旧的全 done 清单读成"还有活没干"，一句问候就"续跑"几天前的任务并沿编号自我强化（2026-09-11 事故：模型坚称 #4–#7 未完成，还去核文件再加倍下注）。修复：注入只含 pending/in_progress 项；**全部完成时整段不注入**（不留编号锚点）；部分完成时附无编号的 `(N of M already done)` 提示。store / WebUI 面板 / 显式 `todo list` 输出全部不变
- **path_guard**：段首 token 被无条件当"程序名"跳过路径校验，`/e/apps/blender.exe --version` 这类**绝对路径可执行文件**逃过全部防线——而同一条路径作参数（`ls /e/apps/...`）却必审，安全口径两套。修复：段首 token 若为路径形态照常纳入校验，裸程序名（`ls`/`git`）行为不变
- **startup**：全新机器上 `llaia serve` / `chat` 首启动直接崩溃「系统找不到指定的路径 (os error 3)」（v0.2 引入迁移逻辑起即坏，Windows ERROR_PATH_NOT_FOUND / Unix ENOENT）。根因：`prepare_startup_dir` 要求迁移先于模板补齐，但 `migrate_if_needed` 的「无旧文件」分支在 `~/.llaia` 尚不存在时就写 `.migrated_v0.2` 标记，`?` 上抛令 `init_scaffold` 的建目录逻辑永远到不了。修复：迁移入口先 `create_dir_all(config_dir)`，并补回归测试（对不存在的目录跑 `prepare_startup_dir`），坐实「裸 `llaia serve` 全新机器可直接跑」的文档承诺
...
**Bug fixes / 稳定性**
- **approval**：`/ok` 批准的 workspace 外操作在执行层被白名单**二次拒绝**，审批形同虚设。修复：`Tool` trait 新增 `execute_approved`，terminal / file_* 覆写为跳过越界白名单（用户已见过完整操作并批准）；命令策略与危险路径黑名单兜底保留
- **file_edit**：`old_string not found` 从裸报错变成能自我纠正的报错——0 命中时附带行号的文件摘录提示重新 `file_read`；行尾单纯不同（CRLF/LF，新克隆 Windows 工作树的典型坑）时换算重试一次，写回只替换命中片段
- **tool_call**：标签降级模式下 ```` ```json ```` 围栏里带 `name` + `arguments` 的调用此前被**静默丢弃**（first-run bootstrap 实测暴露）。现在通用语言名走严格判定（展示数据不会被误执行），未闭合围栏有还原语义；代价如实记录：json 围栏缓冲到闭栏、不再逐字流式。顺带删除零生产调用的 `tool_call/tag_parser.rs`（334 行）
- **todo**：删会话/归档任务线遗留的 `todos/<uuid>.json` 只增不减，改为**启动期垃圾回收**（`TodoStore::gc_orphans()`，按 uuid 对 sessions 表）。刻意决定：归档线清单明确保留；sqlite 查询失败整轮跳过（不压成空列表，防一次报错删光全部）；只启动跑一次
- **path_guard**：Git Bash 风格 `/c/Users/...` 路径被误判越界、workspace 内命令每次弹审批——Windows `PathBuf` 对 MSYS 路径的 `starts_with` 比较必然失败。修复：`convert_msys_path()` 在校验与审批入口统一换算（危险黑名单检查前置，防线不松；多字符首段的 Unix 路径不误换算）
- **webui**：WebUI 发出去的图**从来没进过模型**——`uploads/uploads/...` 双前缀错位 + Windows canonicalize verbatim 前缀漂移，把「路径拼错」报成了「越权逃逸」（自 WS handler 落地起即坏）。修复：base 对齐 CLI、`upload_rel` 兼容两种路径形态、`canonicalize_lenient` 统一两侧形态；三处静默降级收口为一条 WARN
- **webui**：WS 未就绪/断开时发送**静默假死**（异常被点击处理器吞掉、busy 状态卡死）。修复：`wsOpen()` 就绪守卫 + `outbox` 排队（chat/steer/stop 一视同仁，连上自动补发），断开落下 busy 并提示；首版「只提示不排队」实测把竞态转嫁给用户，已改为排队
- **webui**：chat 右侧状态栏**常驻**——推翻同日「两块都空整体隐藏」决策：TODO 经常为空，整栏可见性押在 ENV 上，而 ENV 一失败连 Refresh 恢复入口一起被藏掉。ENV 块常驻并显示失败原因，不再吞异常

**Features**
- **webui**：chat 左侧**活跃会话线列表**（ADR-0031 WebUI 侧通路）——主线 `⌂ Main` 置顶、任务线显示线名 + bound 目录、当前线高亮；点击走 `POST /api/session-lines/switch`（`slash.rs::switch_line_for_web`），复用 `/session` 的切线回灌通路，差异点是 **bound_dir 跟随**：切到任务线同步把 workspace_root 切到 bound_path（此前必经 /move 审批，点击不二次审批），回主线恢复家目录；响应带回目标线尾部消息，屏幕所见与 agent context 严格同一批
- **thinking**：思考能力模型三段交付（[plan](plans/2026-09-10-thinking-capability-model.md)）。**P0 留存**：reasoning SSE 字段 → sqlite 落库 → WebUI 会话详情 + chat 实时折叠思考块（流式期间自动展开、done 收起，思考绝不漏进可见正文）。**P1 档位与门禁**：`[provider.<id>.<model>.thinking]` 能力声明 + `/reasoning` 档位选择，不支持的端点拒收报错不静默降级（终结 Ollama「思考吃满 max_tokens → 重试无效」死循环）。**P2 回传**：`preserve=true` 时历史思考逐字随请求发回（默认关）。明确否决：全矩阵能力表、按模型名猜方言、云端探测
- **terminal**：脚本绕过防护专项收口（grill 2026-09-07 定案）。① **S1 命令行路径提取重写**：引号感知 + flag 语义 + 位置语义，终结 `git commit -m "a/b"` 式误报；② **T3 解释器内联载荷强制人审**：`python -c` / heredoc / `curl | bash` 等（框架三层防线全被绕过的缺口）命中即 `/ok`，`[tools.terminal].interpret_inline` 默认 `approval`；③ **T2 文档化**：[guide/security-hardening.md](guide/security-hardening.md)。不做项留档：`sh -c` 递归解析、OS 级沙箱、缩小爆炸半径（与轻量单 crate 定位冲突）
- **memory**：SOUL 模板补 `# Name` = LLAIA、USER 的 `language` 留空交给 first-run bootstrap 去问（不再替用户预设）；启动时仅升级**逐字节等于旧模板**的占位文件（改过一行就不动，保住 `is_unfilled` 指纹）
- **session**：`/task` 改名 `/session`（旧名保留为转发别名，内部值不动），落地 9/6 事故取证三件套（ADR-0031 修订）：重启仅回主线、任务线显式续接；bound_path 升级为「该线当前所在目录」由 `/move` 维护；`[scope]` 软提示统一；`fork_for_isolated`（cron/委派）pin 家目录不随主线漂移
- **session**：会话线模型收口——主线恒为一条（ADR-0031 定案，grill 2026-09-10 + 拍板 2026-09-11 + 同日实现）。**`/new` 直接删除**（不做别名/弃用过渡）：handler（原 `slash.rs:197`）+ `/help` 文案 + 注释（`sqlite.rs::channel_of`、`todo.rs` 模块头）已清；WebUI 无 `/new` 入口零影响。僵尸 main 行是矛盾源（`latest_session` 永不再选中、WebUI 列表恒可见），造线兜底已有（启动 `latest_session()` 为 None 即建、`switch_to_main` fallback）。新增 **`/archive [days]`**（`slash.rs`）：与 WebUI `archive-older?days=N` 同一条 `archive_messages_before` 通路；无参默认 30 天、钳制 1–3650；0 命中回 no-op 提示；桶隔离性现成继承（`archive:` 前缀 + archived + kind='task' 三重过滤），任务线上同样成立。**归档与 `/clear` 职责正交、不绑定**：`/archive` 纯存储层（live context 零感知）；"换一页并立即遗忘" = `/archive` + `/clear` 配对，不设 `/reset` 聚合命令。**`/clear` 同步清 todo**：`TodoStore::clear_current()`（内存置空 + 删 `todos/<uuid>.json`；无当前 session 静默跳过）。**桶标题动态更新**（`sqlite.rs::archive_messages_before`）：桶每源线一个、跨多次归档累加，每次归档后更新为桶内最新一条消息的日期（`archive of <名> (through <YYYY-MM-DD>)`），替代旧的首写死 cutoff；主线标题恒不变。N=0 同秒竞态不修（`created_at < now` 天然排除同秒末尾，N≥1 是主用例）。测试：`todo.rs::clear_current_*`（清空 + inert 两态）、`sqlite.rs::test_archive_messages_before` 扩动态标题断言、`slash.rs::test_archive_noop_and_clear_todo`（参数边界 + no-op + /clear 集成）。文档同步：AGENTS.md（会话模型 + 斜杠命令清单）、guide/slash-commands.md、guide/memory-and-context.md、glossary.md、ADR-0024（todo 生命周期两处）、ADR-0031（未决节改定案节）、task-to-session plan（处置 6 加注）
- **i18n**：框架消息统一英文（guard 诊断、keepalive、微信媒体占位、CLI/WebUI 提示等）；`qq.rs` 的中文 token 过期字面匹配保留（翻译会破坏检测）
- **session（移除）**：压缩时自动生成会话标题**移除**（辨识度低 + 每次 sidecar 调用）；`sessions.title` 列保留给任务线名
- **webui**：Sessions 页卫生工具「**Archive older [N]d**」——把某条会话 N 天前的消息单事务搬进专属归档接收线（同源复用同一条），源线 id 不变继续当活跃线写，live context 零感知，膨胀的 cron 长线原地瘦身；整线 Delete 组合出「批量删 N 天前」
- **webui**：turn 运行中 Send 变 **Steer**（输入不断路，插话投递后端既有队列）；待传图片从内嵌路径行改为缩略图芯片（可移除）
- **webui**：chat 两栏布局——todo/env 从底栏挪到右侧状态栏（随后一条修订为常驻）；≤900px 窄屏回退折叠
- **webui**：tool 结果可折叠——`tool_result` 回填最近 `tool_start`，渲染为折叠条（点击展开限高滚动），长输出不再撑爆聊天流
- **docs**：一致性扫尾——AGENTS.md「主路径未流式」过时表述、guide 画像模板约定、T/S 编号撞车修正
- **provider**：模型级 `enabled` 开关——参数留在 config 但不进 `/provider` 列表与 WebUI 下拉；**只管可发现性不管可用性**（`provider_from_ref` 不拦，防「关掉当前模型=启动失败」）；引用收敛 `reconcile_disabled_models` 只改内存态，重新启用自动恢复
- **cli**：`LLAIA_HOME` 环境变量 + `llaia serve --host/--port`（覆盖只喂绑定不回写配置；解析优先级 `--config-dir` > `LLAIA_HOME` > `~/.llaia`，空串按未设置处理）
- **webui**：自己发的图以缩略图留在消息气泡里（「否则我都不知道我发了没」）；URL 拼接收进 `fileUrl()`
- **webui**：附件入口改为输入框左侧 **[+]** 按钮（隐藏原生 file input，文件名在芯片行已有）
- **webui**：Config 页各 section 操作按钮集中到固定顶栏——Save 埋在长页面底部的时代结束，保存反馈随按钮上移

**Refactor / 健壮性**
- **robustness**：主干体检机械项（plan.md H4）——40 处生产路径 `lock().unwrap()` 换毒锁恢复（`unwrap_or_else(|e| e.into_inner())`）；常量正则 `unwrap()` 补「编译期写死且合法」注释
