...
- `src/channels/qq.rs`：新增 `QqChannel`（WS 接事件 + HTTPS 发消息 + 分片）
- `src/channels/qq_split.rs`：新增 `split_reply` 纯函数
- `src/config.rs`：`ChannelsConfig` 加 `qq: QqConfig`，新增 `CliChannelConfig`
...
- `tests/qq_http.rs`：`QqChannel` HTTP 发送/重试/解析的 mockito 测试

## 遗留问题（推迟到后续 ADR）

- **环境变量插值**：QQ token 明文存 config.toml，待 ADR 0008 遗留项一起处理
