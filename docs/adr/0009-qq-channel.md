...
P1 完成后用户希望接入 QQ 作为第二个交互频道，主要场景是在手机端通过 QQ 单聊唤醒桌面上的 LLAIA 完成任务查询、记忆追加等。候选方案：
...
QQ 单条消息上限约 2000 字符。LLAIA 取 1800 作为安全阈值，纯函数 `split_reply(text, max)` 按三级 fallback 切分：
